...
static void scsi_dma_restart_bh(void *opaque)
{
    ...
    QTAILQ_FOREACH_SAFE(req, &s->requests, next, next) {
        ...
        if (req->retry) {
            req->retry = false;
            switch (req->cmd.mode) {
            case QEMU_SCSI_XFER_FROM_DEV:
            case QEMU_SCSI_XFER_TO_DEV:
                scsi_req_continue(req);
                break;
            case QEMU_SCSI_XFER_NONE:
...
int scsi_req_parse_cdb(SCSIDevice *dev, SCSICommand *cmd, uint8_t *buf)
{
    ...
    cmd->mode = cmd->xfer ? scsi_cmd_xfer_mode(cmd->buf) : QEMU_SCSI_XFER_NONE;
...
void scsi_req_continue(SCSIRequest *req)
{
    ...
    if (req->cmd.mode == QEMU_SCSI_XFER_TO_DEV) {
...
void scsi_req_data(SCSIRequest *req, int len)
{
    ...
    assert(req->cmd.mode != QEMU_SCSI_XFER_NONE);
    if (!req->sg) {
        req->resid -= len;
    ...
    if (req->cmd.mode == QEMU_SCSI_XFER_FROM_DEV) {
